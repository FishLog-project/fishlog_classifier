...
## 2026-08-18 — `기타` 층화 분할 (split.py)

`기타` recall이 **val 43.2% vs test 71.05%** 로 28pp 벌어졌다. 표본이 각각 37/38장이라
우연으로 보기 어려웠는데, 원인은 모델이 아니라 **시험지 구성**이었다.

`기타`에는 난이도가 전혀 다른 두 부류가 섞여 있다:

| 부류 | 파일 접두사 | 예 | 난이도 |
|---|---|---|---|
| 24종 밖 **어종** | `inat_*` | 학꽁치·붕장어·빙어·가오리 | **어려움** (진짜 물고기라 24종과 헷갈림) |
| 비물고기 | `web_*` | 회 접시·낚싯대·고양이·하늘 | 쉬움 |

무작위 분할 결과 이 비율이 split마다 튀었다:
...
→ `split_species(stratify=True)` 로 `source_stratum` 별 독립 배정 후 합친다.
   **`기타`에만 적용**한다(`--no-stratify-other` 로 이전 동작 재현 가능).
   어종 24종은 출처가 섞여도 난이도 차이가 이만큼 크지 않다.

### 왜 이게 중요한가

층화 전에는 **val과 test의 `기타` 지표를 비교할 수 없었다.** 개선 여부를 판단하려면
시험지가 같아야 한다. 앞으로 `기타`를 보강할 때도 이 층화가 있어야 효과를 측정할 수 있다.

### 근본 원인은 `기타`가 너무 작다는 것

`config.SPECIES["기타"].target` 은 1,000장인데 실제는 249장이다.
4버킷 × 3분할 = 12칸에 249장을 나누면 칸당 약 20장이라, 무작위 배정의 편차가 클 수밖에 없다.
층화는 증상을 눌러줄 뿐이고 **수집으로 절대량을 늘려야 한다.**

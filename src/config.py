...
CONFIDENCE_THRESHOLD = 0.25  # top1 < 임계값이면 uncertain=true → "다시 촬영" 유도
# 0.45(초기 추정) → 0.25 (2026-08-20, val 1,117장 서빙 경로 실측).
# 0.45면 어종 사진의 22.3%가 거부돼 기준(10%)을 두 배 넘겼다.
# 재튜닝: `python -m scripts.tune_threshold` (labels.json 과 이 값을 함께 맞출 것)
